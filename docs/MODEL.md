...
每个 spot 以图像坐标为中心，裁取边长 224、896、1792 像素的三个视野，均缩放为 224×224。三幅图共享 CNN 编码器，拼接特征后预测 200 个基因的相对表达。
...
训练标签按 A1+B1 的逐基因均值和标准差进行标准化。损失为 MSE 与 0.2 倍批内基因相关性损失之和。使用 AdamW、学习率预热与余弦衰减、梯度裁剪，以及颜色、旋转和翻转增强。

[configs](../configs)中的三组配置均采用 ImageNet 初始化，编码器和预测头一起训练。每组在 C1 比较当前参数与指数移动平均参数（EMA），以及单次预测与四旋转平均（TTA）。
