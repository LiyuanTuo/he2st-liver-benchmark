"""固定 C1 验证集上的 Stem 采样消融；选定后才运行 D1，保留旧预测。

WSL: python3 scripts/28_stem_inference_audit.py
...
OUT = ROOT / "results/improvement_20260908/stem"
...
def main():
    ...
    upper = float(expression.max())  # 上界仅从训练标签得来。
    checkpoint = torch.load(ROOT / "results/stem_adapted/best.pt", map_location="cpu", weights_only=True)
    model = Stem_models["Stem"](input_size=200, depth=12, hidden_size=384, num_heads=6, label_size=512).cuda().eval()
    step_count = checkpoint["epoch"] * (len(features)//32)
    diagnostics = {"epoch": checkpoint["epoch"], "estimated_updates": step_count,
                   "initial_ema_weight_fraction": 0.9999**step_count,
    ...
    def sample(data, branch, clipping, steps, repeats):
        ...
        for repeat in range(repeats):
            ...
            for start in range(0,len(conditions),64):
                ...
                # 稳定的 bf16 避免 fp16 溢出；所有对照使用完全相同精度和噪声。
    ...
    trials = []
    for branch, clipping in [("ema","image_range"),("ema","training_range"),("model","image_range"),("model","training_range"),("model","none")]:
        start = time.perf_counter()
        prediction = sample(subset,branch,clipping,50,1)
        metrics = score(subset["raw_counts"],prediction)
        trials.append({"branch":branch,"clipping":clipping,"validation_subset":metrics,"seconds":time.perf_counter()-start})
        save_json("ablation.json",trials)
        print(trials[-1],flush=True)
    selected = max(trials,key=lambda t:t["validation_subset"]["pcc"])
    # 先落盘选择结果，再加载 D1，防止拿测试分数挑分支。
    ...
    for slide in ["C73_C1","C73_D1"]:
        ...
        np.savez_compressed(OUT/f"{slide}_predictions.npz",predicted=prediction,
                            true=data["log_normalized"],coordinates_xy=data["coordinates_xy"],genes=data["genes"])
